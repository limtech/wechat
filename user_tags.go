...
// new user tag
func NewUserTag(accessToken string) *UserTag {
	return &UserTag{
...
// 创建标签
func (self *UserTag) Create(name string) (UserTagData, error) {
	data := UserTagData{
		Tag: UserTagItem{
			Name: name,
		},
...
// 获取公众号已创建的标签
func (self *UserTag) GetList() (UserTagList, error) {
...
// 编辑标签
func (self *UserTag) Update(id int64, name string) (UserTagData, error) {
	data := UserTagData{
		Tag: UserTagItem{
			Id:   id,
			Name: name,
...
// 删除标签
func (self *UserTag) Delete(id int64) error {
	data := UserTagData{
		Tag: UserTagItem{
			Id: id,
		},
...
// 获取标签下粉丝列表
func (self *UserTag) GetUserList(tagId int64, nextOpenid string) (UserTagUserListData, error) {
...
// 批量为用户打标签
func (self *UserTag) BatchTagging(tagId int64, openid []string) error {
	// get remote data
	res, err := HttpPostJson(
...
// 批量为用户取消标签
func (self *UserTag) BatchUnTagging(tagId int64, openid []string) error {
	// get remote data
	res, err := HttpPostJson(
...
// 获取用户身上的标签列表
func (self *UserTag) GetUserTagIds(openid string) (UserTagIdList, error) {
